...
# Delete restaurant function using session to get
# the current user for permissions
def rest_del(restaurant_id):
    current_user_id = session.get("user_id")
    current_user = User.query.get(current_user_id)
    rest_to_delete = Restaurant.query.filter_by(
...
# Edit restaurant function using session to get
# the current user for permissions
def rest_edit(restaurant_id):
    current_user_id = session.get("user_id")
    current_user = User.query.get(current_user_id)
    rest_to_edit = Restaurant.query.filter_by(
        restaurant_id=restaurant_id).first()
    if rest_to_edit:
        if request.method == "POST":
        ...
        return render_template(
            "rest_edit.html", restaurant=rest_to_edit, current_user=current_user
        )
    else:
        flash("Restaurant not found")
    return redirect(url_for("rest"))
...
# Changed the cookies with session to store the user login
# status and data more secured not visible to user
# secured doubling the email
def register():
    if request.method == "POST":
        print(request.form)
...
# Logout works by deleting the cookies and session
# data that stores the user login status and data
...
def login():
    ...
    if request.method == "POST":
        ...
        # Now login create session instead of cookies because
        # it is more secure and not visible to user
...
def menu_add(restaurant_id):
    ...
    if request.method == "POST":
        ...
        new_menu_item = Menu(
            restaurant_id=restaurant_id, dish_name=dish_name, price=price
        )
    ...
    return render_template(
        "menu_add.html", restaurant=restaurant, current_user=current_user
    )
